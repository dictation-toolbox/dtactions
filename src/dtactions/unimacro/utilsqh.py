...
"""
#pylint:disable=C0116, C0302,  W0613, R0912, R0914, R0915, W0702
import sys
import itertools
import unicodedata
import types
...
def getWindowWithTitle(wantedTitle, checkendstring=None):
    ...
    foundHandle = None
    if checkendstring:
        win32gui.EnumWindows(lookForWindowTextEnd, wantedTitle)
    else:
        win32gui.EnumWindows(lookForWindowText, wantedTitle)
...
def GetProcIdFromWnd(wHndle):
    ...
    while 1:
        ...
        if nextParent == 0 or nextParent == parent:
            break
        parent = nextParent
    wHndle = parent
...
def lookForWindowTextEnd(hwnd, text):
    ...
    if not getwindowtext:
        return 1
    if getwindowtext.endswith(text):
        foundHandle = hwnd
        return
    return 1
...
if sys.platform != 'linux2':
    ...
    def getIEProcesses(filterClasses=None):
        ...
        ClassName:
            CabinetWClass == My Computer  ## now the default!!
            ExploreWClass == Windows Explorer  ## seems to be obsolete
            IEFrame == Internet Explorer  ## seems to be obsolete
    
>>> getIEProcesses()
>>> getIEProcesses(filterClasses=['IEFrame'])
[]
        """
        filterClasses = filterClasses or ['CabinetWClass']
        ShellWindowsCLSID = '{9BA05972-F6A8-11CF-A442-00A0C90A8F39}'
        ShellWindows = win32com.client.Dispatch(ShellWindowsCLSID)
        L = []
        for s in ShellWindows :
            ClassName = GetClassName(s.HWND)
            if ClassName in ['CabinetWClass', 'ExploreWClass', 'IEFrame']:
                if ClassName in filterClasses:
                    L.append((str(s.LocationName), str(s.LocationURL), s.HWND, ClassName))
            else:
                print("getIEProcesses, unexpected ClassName: %s"% ClassName)
        return L
...
##    """
    ...
    if abs(dateIn - dateOut) < 0.00001:
        ...
        if filecmp.cmp(str(inf), str(out)):
